...
public class DataFiles {
  ...
  public static DataFile fromInputFile(
    ...
    FileFormat format = FileFormat.fromFileName(location);
    return new GenericDataFile(
        location, format, partition, file.getLength(), DEFAULT_BLOCK_SIZE, metrics, keyMetadata);
  }
  ...
  public static DataFile fromStat(
      FileStatus stat, PartitionData partition, Metrics metrics, EncryptionKeyMetadata keyMetadata) {
    String location = stat.getPath().toString();
    FileFormat format = FileFormat.fromFileName(location);
    return new GenericDataFile(
        location, format, partition, stat.getLen(), stat.getBlockSize(), metrics, keyMetadata);
  }
