...
public class CryptoStreamReader {

  public static SeekableInputStream decrypt(
      SeekableInputStream original, PhysicalEncryptionKey key) {
    KeyMaterial keyMaterial = KeyMaterials.from(
        key.keyMetadata().keyAlgorithm(),
        ByteBuffers.toByteArray(key.secretKeyBytes()),
        ByteBuffers.toByteArray(key.iv()));
    return SeekableInputCompat.of(
        CryptoStreamFactory.decrypt(
            SeekableInputCompat.of(original),
            keyMaterial,
            key.keyMetadata().cipherAlgorithm()));
  }

  public static InputFile decrypt(InputFile original, PhysicalEncryptionKey key) {
  ...
  private static final class DecryptingInputFile implements InputFile {
    ...
    DecryptingInputFile(
        InputFile delegate, PhysicalEncryptionKey key) {
      this.delegate = delegate;
      this.key = key;
    }
